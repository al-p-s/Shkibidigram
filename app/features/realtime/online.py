...
async def set_online(user_id: str) -> None:
    await (await get_redis()).setex(ONLINE_KEY.format(user_id=user_id), ONLINE_TTL, "1")


async def set_offline(user_id: str) -> None:
    await (await get_redis()).delete(ONLINE_KEY.format(user_id=user_id))


async def is_online(user_id: str) -> bool:
    return bool(await (await get_redis()).exists(ONLINE_KEY.format(user_id=user_id)))
